...
function get_window_time_difference(interval: number) {
	...
	for (let i = 1; i <= interval / slot_size; i++) {
		...
		if ((ms_since_midnight % interval) < slot_size) {
			...
			window_time.setUTCMinutes(5 * Math.floor(window_time.getUTCMinutes() / 5), 0);
			...
			const secs = ("0" + Math.floor((difference % _minute) / _second)).slice(-2);
		...
		return m(".farmclock", [
			...
			m(".plantdata", [
				m("table.preset", [
					...
					m("tbody", plant_data.slice().reverse().map(data =>
							data.plants.slice().reverse().map(plant =>
